...
/** Verbose functions */
static void verboseLeft(void)
{
	char *path = kmalloc(sizeof(char) * 25, GFP_KERNEL);
	char *argv[] = { "/usr/bin/aplay", NULL, NULL };
	static char *envp[] = {
        "HOME=/",
        "TERM=xterm",
	"ALSAPCM=ES1371/1",
	...
	path = "/etc/verbose/left.wav\0";

	argv[1] = path;
 
	call_usermodehelper( argv[0], argv, envp, UMH_NO_WAIT );
}
static void verboseMiddle(void)
{
	char *path = kmalloc(sizeof(char) * 25, GFP_KERNEL);
	char *argv[] = { "/usr/bin/aplay", NULL, NULL };
	static char *envp[] = {
	...
	path = "/etc/verbose/wheel.wav\0";

	argv[1] = path;
 
	call_usermodehelper( argv[0], argv, envp, UMH_NO_WAIT );
}
static void verboseRight(void)
{
	char *path = kmalloc(sizeof(char) * 25, GFP_KERNEL);
	char *argv[] = { "/usr/bin/aplay", NULL, NULL };
	static char *envp[] = {
 ...
 */
	...
	/* Verbose caller */
	if((packet[0] & 1) == 1) verboseLeft();
	if(((packet[0] >> 1) & 1) == 1) verboseRight();
	if(((packet[0] >> 2) & 1) == 1) verboseMiddle();
